...
另两处工作区治理观察（非代码缺陷）：根目录物理残留 10 个 `.vsix`（`.gitignore` 已排除、不入库）与 `.sim-tmp-qa-pub/` 遗留目录（`.gitignore` 只写了 `.sim-tmp/`，未覆盖该目录；`.vscodeignore` 的 `.sim-*/**` 已保证不入包）。
...
| ⑦ | 工作区治理：清理 `.sim-tmp-qa-pub/` 遗留目录；`.gitignore` 补 `.sim-*/` 通配；历史 `.vsix` 归档策略确认 | 【部署专家】 |
