...
| 5 | `@types/vscode ^1.99.0`（devDependencies）vs `engines.vscode ^1.134.0` | 两者不匹配：类型层面按 1.99 编译，引擎门槛却要求 ≥1.134（README 自述"本机 1.134 已满足"） | 1.99–1.133 的 VSCode 会被 engines 拒装，但代码只按 1.99 类型层面保证编译；属版本声明技术债（见 §9-⑤） |
...
- **`@types/vscode` 漂移**：类型层面（1.99）与引擎门槛（1.134）长期背离会在未来某次 API 升级时突然暴露（§9-⑤）。
