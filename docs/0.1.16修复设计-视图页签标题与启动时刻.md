...
- **覆盖范围**：writeRegistry 全部 5 个 landing 点（step1 re-adopt / step2 残留迁移 / step2 外部接管 / step4 等待后接管 / launchManaged）自动覆盖，零新增调用点。
...
| 无头探针 `node scripts\sim.mjs` | **ALL PASS, 9 SKIP** | 本轮实测（8/8 聚合检查 + 全族零失败）；SKIP 均为沙箱已知边界（netstat/CIM） |
...
4. **沙箱与真实实例边界**：`Get-Process` 的真机可用性由 E-FX-1 先例实证（0.1.15 轮定案 2392 启动时刻），沙箱内 CIM 不可用边界在案（E-SANDBOX）；#96 实现的全部无头验证走 mock 注入面（PP-12-3），真机行为归 RV-17-2 终验。本轮零新增探针被拒绝的测试用例。
