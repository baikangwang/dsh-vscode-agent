...
> 敏捷模式编排人格（预设内建，非外部插件）会话开始时自动读取、派发时按角色内联，**无需编写任何 host 逻辑**。
...
| **B** | `.dsh/contracts/<role>/*.md` | 结构化**文本** | 派发该角色子代理时由编排者内联进其 prompt | 可叙事、需模型理解的契约：设计模板、编码约束、门控标准、语义约定 |
...
- **按角色**：编排者调度到某角色时，编排者读该角色 `.dsh/contracts/<role>/` + `_shared` 基线，内联进该 subagent 的 prompt；未调度角色不加载 → 上下文受控、token 不浪费。
