...
> **由 `profile.yaml` 的 `tech_stack` 决定是否适用**——profile 未声明该技术栈时，本节整体**不适用**，
> **读者自行跳过即可，不要删除本节**。
>
> **为什么"不要删除"（2026-09-20 第十二轮续修正）**：本节原先写的是"**由架构师在接入时删除本节**"，
> 而那与交付方式**直接冲突**——契约是**逐字节复制**到消费项目的，正本一旦被某个项目改过，
> 它就与正本不一致，`project-conformance-check` 会判 FAIL，下一次同步又会把它改回来。
> **用"让读者改文件"来表达"条件不适用"是错的手段**：条件本来就该在**读的时候**判定，
> 而不是在**写的时候**用删改去实现。现在写成"不适用则跳过"，正本对任何项目都保持逐字节一致。
