...
// 依据：`敏捷管理理论基础调研报告-v2.0-2026-09-18.md`（**模式仓库 `docs/` 下，不是交付件**） §5.1（GQM，Basili & Weiss 1981）对本模式的硬要求——
//   「本模式若要做度量，第一步必须是 GQM 的 Goal 层，而不是选指标……
//     没有这一行的度量，按 GQM 的判断，从一开始就是无效度量。」
// `_shared/metrics-gqm.md` 承载 Goal→Question→Metric 对照；**台账加了列而没人问它回答什么问题，
// 这里必须报错**——否则那张表会漂成摆设（本模式反复出现"文档声称与文件不符"的那类失效）。
